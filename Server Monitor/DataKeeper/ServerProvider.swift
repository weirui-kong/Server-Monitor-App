...
class ServerProvider: ObservableObject{
    ...
    static var monitors: [(String, String, String, MonitorType, Bool)] = []
    ...
    private func updateServerMonitorData() {
        ...
        for monitor in ServerProvider.monitors.filter({ $0.4 }) {
            // 发送网络请求获取监视器数据
            NetworkManager.shared.sendRequest(path: monitor.1) { data in
                // 使用监视器对应的工厂方法解析数据并创建 UniversalServerGroup 实例
                let group: UniversalServerGroup? = monitor.3.factory(data, monitor.0)
                ...
                let group = UniversalServerGroup(label: monitor.0, server: [], lastUpdated: 0, type: monitor.3)
    ...
    private func saveMonitorsToUserDefaults() {
        // 将数据转换为可保存的格式
        let data = ServerProvider.monitors.map { (name, url, method, type, show) -> [String: Any] in
            return [
                "name": name,
                "url": url,
                "method": method,
    ...
    private func loadMonitorsFromUserDefaults(){
        ...
        // 将数据转换为原始格式
        ServerProvider.monitors = data.compactMap { dict -> (String, String, String, MonitorType, Bool)? in
            ...
            return (name, url, method, type, show)
        }
    }
    
    // 添加新的数据到 monitors
    func addMonitor(name: String, url: String, method: String, type: MonitorType, show: Bool) {
        // 添加新的数据
        ServerProvider.monitors.append((name, url, method, type, show))
